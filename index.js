...
const ORIGIN_MAX_BYTES = parseInt(
  process.env.ORIGIN_MAX_BYTES || "10485760",
  10,
); // 10 MB default
const OUTPUT_MAX_BYTES = parseInt(
  process.env.OUTPUT_MAX_BYTES || "10485760",
  10,
); // 10 MB default
const SHARP_MAX_PIXELS = parseInt(
  process.env.SHARP_MAX_PIXELS || "16000000",
  10,
); // ~16 MP
...
const LOCK_WAIT_TIMEOUT_MS = parseInt(
  process.env.LOCK_WAIT_TIMEOUT_MS || "10000",
  10,
);
const LOCK_RETRY_DELAY_MS = parseInt(
  process.env.LOCK_RETRY_DELAY_MS || "150",
  10,
);
...
if (!preferredAllowed) {
  ...
  console.warn(
    "[DEPRECATION] Use ALLOWED_DOMAINS instead of ALLOWED_PREFIXES. Backward-compatible for now.",
  );
}
...
if (ALLOWED_RAW.length === 0) {
  throw new Error(
    "ALLOWED_DOMAINS env variable is required (comma-separated list).",
  );
}
...
app.get("/", async (req, res) => {
  try {
    ...
    const allowed =
      (ALLOWED_URL_PREFIXES.length > 0 &&
        ALLOWED_URL_PREFIXES.some((p) => urlStr.startsWith(p))) ||
    ...
    if (!redisReady) {
      ...
      try {
        const cachedImg = await redisClient.sendCommand(["GET", cacheKey], {
          returnBuffers: true,
        });
        if (cachedImg) {
          const ok =
    ...
    const tryAcquireLock = async () => {
      try {
        const ok = await redisClient.set(lockKey, "1", {
          NX: true,
          EX: LOCK_TTL_SECONDS,
        });
        return ok === "OK";
    ...
    if (redisReady && !haveLock) {
      ...
      while (Date.now() - startWait < LOCK_WAIT_TIMEOUT_MS) {
        try {
          const cached = await redisClient.sendCommand(["GET", cacheKey], {
            returnBuffers: true,
          });
          if (cached) {
    ...
    logger.info(
      `Cache miss: ${cacheKey}, fetching with url: ${originUrl.toString()}`,
    );
    ...
    if (redisReady && !haveLock) {
      ...
      try {
        const cachedAfterWait = await redisClient.sendCommand(
          ["GET", cacheKey],
          { returnBuffers: true },
        );
        if (cachedAfterWait) {
    ...
    class ByteLimit extends Transform {
      ...
      _transform(chunk, enc, cb) {
        ...
        if (this.total > this.limit) {
          cb(
            Object.assign(new Error("ORIGIN_MAX_BYTES_EXCEEDED"), {
              code: "ORIGIN_MAX_BYTES_EXCEEDED",
            }),
          );
        } else {
          cb(null, chunk);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          `Origin fetch failed: ${originUrl.toString()} [${response.status}] ct=${ct || "n/a"}`,
        );
        try {
          if (redisReady && haveLock) await releaseLock();
        } catch (_) {}
      ...
      if (clHeader) {
        ...
        if (Number.isFinite(cl) && cl > ORIGIN_MAX_BYTES) {
          logger.warn(
            `Origin content-length too large: ${cl} > ${ORIGIN_MAX_BYTES}`,
          );
          try {
            if (redisReady && haveLock) await releaseLock();
          } catch (_) {}
      ...
      if (optimizedBuffer.length > OUTPUT_MAX_BYTES) {
        logger.warn(
          `Output buffer too large: ${optimizedBuffer.length} > ${OUTPUT_MAX_BYTES}`,
        );
        try {
          if (redisReady && haveLock) await releaseLock();
        } catch (_) {}
      ...
      logger.info(
        `Image processed: webp, final size=${optimizedBuffer.length} bytes`,
      );
      ...
      if (e && e.code === "ORIGIN_MAX_BYTES_EXCEEDED") {
        logger.warn(
          `Origin stream exceeded max bytes (stream): limit=${ORIGIN_MAX_BYTES}`,
        );
        try {
          if (redisReady && haveLock) await releaseLock();
        } catch (_) {}
      ...
      if (isAbort) {
        logger.warn(
          `Origin fetch timeout after ${ORIGIN_TIMEOUT_MS}ms: ${originUrl.toString()}`,
        );
        try {
          if (redisReady && haveLock) await releaseLock();
        } catch (_) {}
